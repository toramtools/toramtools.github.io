'''
# Snippet to add "CH" to each story
mq_data = original_js_data()
new_mq = []
ch = 0
for name, xp in mq_data.items():
	if name.startswith('Chapter'):
		ch+=1
		continue
	new_mq.append(["CH{} - {}".format(ch, name), xp])
'''
...
def xp_required (iLv, iLvP, tLv, tLvP):
    xp = 0
